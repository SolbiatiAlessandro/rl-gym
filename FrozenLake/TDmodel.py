...
class TDmodel():
    def __init__(self, model, env_size, gamma, alpha, verbose=False):
        """
        model: 'SARSA','Q'
    ...
    def choose_action(self, state, epsilon=0.5, epsilon_greedy=False):
        ...
        if epsilon_greedy:
            if random.random() < epsilon:
                ...
                self.P[state] = np.random.choice(np.delete(np.arange(env.action_space.n), np.argmax(self.Q[state])))

        elif not epsilon_greedy:
